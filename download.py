...
BASE_URL = "https://www.ebi.ac.uk:443/interpro/api/taxonomy/uniprot/entry/pfam/PF00930/?page_size=200"
...
def output_list():
    ...
    while next:
        ...
        for i, item in enumerate(payload["results"]):
            ...
            sys.stdout.write(
                parse_column(
                    item["metadata"]["source_database"], "metadata.source_database"
                )
                + "\t"
            )
            sys.stdout.write(
            ...
            sys.stdout.write(
                parse_column(item["metadata"]["parent"], "metadata.parent") + "\t"
            )
            sys.stdout.write(
                parse_column(item["metadata"]["children"], "metadata.children") + "\t"
            )
            sys.stdout.write(
                parse_column(
                    item["extra_fields"]["counters"]["entries"],
                    "extra_fields.counters.entries",
                )
                + "\t"
            )
            sys.stdout.write(
                parse_column(
                    item["extra_fields"]["counters"]["proteins"],
                    "extra_fields.counters.proteins",
                )
                + "\t"
            )
            sys.stdout.write("\n")
